    ...
    switch l.type
        case 'conv'
            res(i+1).x = vl_nnconv(res(i).x, l.weights{1}, l.weights{2}, ...
                ...
                cudnn{:}) ;
        case 'concat'
            if 0 %removed by Lina
                sigmaMap   = bsxfun(@times,ones(size(res(i).x,1),size(res(i).x,2),1,size(res(i).x,4),'single'),permute(sigma,[3 4 1 2]));
                res(i+1).x = vl_nnconcat({res(i).x,sigmaMap});
            end
            if 1 %added by Lina
                if size(sigma,1)~=size(res(i).x,1)
                    sigmaMap   = bsxfun(@times,ones(size(res(i).x,1),size(res(i).x,2),1,size(res(i).x,4)),permute(sigma,[3 4 1 2])) ;
                    res(i+1).x = vl_nnconcat({res(i).x,sigmaMap}) ;
                else
                    res(i+1).x = vl_nnconcat({res(i).x,sigma}) ;
                end
            end
